from .util import *
from ..util import SMALL_CONST, LARGE_CONST
...
class KSD(object):
    ...
    def kernel_temp(self, x, mute=False):
        """
    ...
    def kappa(self, x, sa=None, mute=True):
        """
        Computes the KSD kappa matrix.
        """
        kernel_mat = self.kernel(x, x)  # (n, n)
        assert is_symmetric(kernel_mat)
        # Conditional version here
        score_mat = self.score(x, sa=sa)  # (n, d)

        if not mute:
            self.logger.info("\nComputing kxx, k_xx, k_x_x ...")  # Heavy
        kernel_res = self.kernel_temp(x, mute=mute)
        ...
        ksd_stat = np.mean(res)
        if ksd_stat + SMALL_CONST < 0:
            ind = np.unravel_index(np.argmin(res), res.shape)
            self.logger.warning("Negative KSD kernel matrix!"
                                f"Min value: {res[ind]}"
                                f" terms: {term1[ind]}, {term2[ind]}, {term3[ind]}, {term4[ind]}"
                                f"term1 {score_mat.dot(score_mat.T)} det {np.linalg.det(score_mat.dot(score_mat.T))}"
                                f"\nkernel_mat {kernel_mat}"
                                f"\n score_mat {score_mat}\n"
                                f"mean {np.mean(res)}")
        elif ksd_stat > LARGE_CONST:
            ind = np.unravel_index(np.argmax(res), res.shape)
            self.logger.warning(f"KSD is unbounded {ksd_stat}"
                                f"Min value: {res[ind]}"
                                f" terms: {term1[ind]}, {term2[ind]}, {term3[ind]}, {term4[ind]}"
                                f"term1 {score_mat.dot(score_mat.T)} det {np.linalg.det(score_mat.dot(score_mat.T))}"
                                f"\nkernel_mat {kernel_mat}"
    ...
    def compute_kappa(self, samples, sa=None):
        """
        ...
        kappa_vals = self.kappa(samples, sa=sa)
