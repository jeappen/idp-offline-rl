...
def most_common(cnt):
    ...
    most_common_elements = [i for i in list(cnt.keys()) if cnt[i] == max_count]
...
def normalize(counts, alpha=0.):
    ...
    if isinstance(counts, Counter):
        ...
        temp = sum(counts.values()) + alpha * len(list(counts.keys()))
        ...
        temp = sum(counts.values()) + alpha * len(list(counts.keys()))
        dist = {key: (counts[key] + alpha) / temp for key in list(counts.keys())}
