...
def evaluate_on_spec_and_environment(
    env: gym.Env, spec_eval, n_trials: int = 10, epsilon: float = 0.0, render: bool = False
) -> Callable[..., float]:
    """Returns scorer function of evaluation on environment for a given STL spec from diff-spec..
    ...
    Args:
        env: gym-styled environment.
        spec_eval : function on trajectory giving a real number evaluating the spec
    ...
    """
    # Import here since diff-spec is optional requirement for LCB algo
    ...
    def scorer(algo: AlgoProtocol, *args: Any) -> float:
        ...
        # Evaluate the trajectory on the spec
        return spec_eval(default_tensor(np.array([trajectory]))).detach().numpy()[0]
